...
Every tasty fruit basket needs also pineapples, so you decide to add five pineapples to your oranges. As a data analyst, your reflex is to immediately create the variable `my_pineapples` and assign the value 5 to it. Next, you want to calculate how many pieces of fruit you have in total. Since you have given meaningful names to these values, you can now code this in a clear way: 

```
my_pineapples + my_oranges
```

*** =instructions
- Assign to `my_pineapples` the value 5.
- Add the variables `my_pineapples` and `my_oranges` and have R simply print the result.
- Assign the result of adding `my_pineapples` and `my_oranges` to a new variable `my_fruit`.

*** =hint
`my_fruit` is just the sum of `my_pineapples` and `my_oranges`. You can use the `+` operator to sum the two and `<-` to assign that value to the variable `my_fruit`.
...
my_pineapples + my_oranges
# Create the variable my_fruit
my_fruit <- my_pineapples + my_oranges
```

*** =sct
```{r}
test_object("my_pineapples", incorrect_msg = "Keep the line that assigns 5 to `my_pineapples`.")
test_object("my_oranges", incorrect_msg = "Keep the line that assigns 7 to `my_oranges`.")
test_output_contains("my_pineapples + my_oranges",
                     incorrect_msg = "Make sure to print out the result of adding `my_pineapples` and `my_oranges`. The code example in the description already gives away the answer to this instruction!")
msg <- "Have you used `my_fruit <- my_pineapples + my_oranges` to create the `my_fruit` variable?"
test_object("my_fruit", undefined_msg = msg, incorrect_msg = msg)
success_msg("Nice one! The great advantage of doing calculations with variables is reusability. If you just change `my_pineapples` to equal 12 instead of 5 and rerun the script, `my_fruit` will automatically update as well. Continue to the next exercise.")
```
